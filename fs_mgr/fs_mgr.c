...
#define FS_OPTIONS_LEN 1024
    ...
    fstab->dualboot = false; //set default value is false 
	...
	int code = 0;
	if (code = stat(fstab->recs[cnt].blk_device, &fstab->recs[cnt].st) != 0)
		ERROR("stat: Error %d on file %s\n", code, fstab->recs[cnt].blk_device);

	if (strcmp(fstab->recs[cnt].mount_point, "/system1") == 0)
		fstab->dualboot = true; 
	//end 
